...
export const SKELETON_CSS = `
  ...
  /* The hand-off: real rows come in from the top down as the shimmer goes.
     Deliberately small — 6px and a third of a second. A reveal big enough to
     notice as an animation is a reveal you have to wait through every time
     the page loads. */
  @keyframes schoolagyReveal {
    from { opacity: 0; transform: translateY(6px); }
    to   { opacity: 1; transform: none; }
  }
  /* Table rows get opacity only. transform on a <tr> is legal but renders
     inconsistently across engines, and a row that shifts independently of its
     own cell borders looks broken. */
  @keyframes schoolagyRevealRow {
    from { opacity: 0; }
    to   { opacity: 1; }
  }
  @media (prefers-reduced-motion: reduce) {
...
/**
 * Animates the real rows in, top to bottom, once the page's own script has
 * filled the containers the skeleton was holding.
 *
 * Ordered by actual vertical position rather than by container, so the cascade
 * reads as one wave down the page instead of several columns racing each
 * other — Home fills a grades table and two assignment lists side by side, and
 * staggering each independently looks like a glitch rather than a reveal.
 *
...
export function revealContent(root: HTMLElement, pageId?: string): void {
  const sels = pageId ? SKELETON_SELECTORS[pageId] : undefined;
  if (!sels) return;
  ...
  rows.forEach((el, i) => order.set(el, i));
  rows.sort((a, b) => {
    const d = (tops.get(a) ?? 0) - (tops.get(b) ?? 0);
  ...
  rows.forEach((el, i) => {
    ...
    // Capped so a long list still finishes promptly — 40 contacts at 28ms
    // each would otherwise take over a second to finish arriving.
    const delay = Math.min(i * 28, 520);
    el.setAttribute("data-reveal", "");
    el.style.animation =
      `${isRow ? "schoolagyRevealRow" : "schoolagyReveal"} 340ms cubic-bezier(0.22, 0.61, 0.36, 1) ${delay}ms both`;
